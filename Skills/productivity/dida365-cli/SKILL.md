...
当你需要帮用户添加任务到滴答清单时，请严格遵循以下两步流程：

1. **获取目标清单（Project）的 ID：**
   不能直接凭猜测使用清单名字（如 `inbox` 或 `阅读列表`），必须先查询出具体的 `projectId`。
   ```bash
   dida365 project list --json
   ```
   *从返回结果中找到目标清单（如 "收集箱"、"📖阅读列表"、"买东西"），复制对应的 `id`。如果你需要把任务放进默认收集箱，请用 `dida365 sync all` 找到 `inboxId`。*

2. **创建任务并指定 Project ID：**
   使用获取到的 `id` 执行创建任务。如果你需要附加链接或额外信息，请使用 `-c`（或 `--content`）参数放入描述中。
   ```bash
   dida365 task create "任务标题" -p "获取到的projectId" -c "这里是任务的备注内容或链接"
   ```
