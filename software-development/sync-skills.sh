...
# Clean up old conflict flag if resolved
[ -f "$CONFLICT_FILE" ] && echo "[sync-skills] Found pending conflict flag, triggering resolution..." && exit 0
...
if [ "$LOCAL" != "$REMOTE" ]; then
    ...
    if git pull --rebase=merges -X theirs origin main 2>"$CONFLICT_FILE.pull"; then
        ...
        # Write conflict report for Hermes to handle
        cat > "$CONFLICT_FILE" <<EOF
{
  ...
  "remote_sha": "$REMOTE",
  "repo": "$SKILLS_DIR",
  "error": "$(cat "$CONFLICT_FILE.pull" | head -20 | tr '\n' ' ' | sed 's/"/\\"/g')",
